...
struct Obstacle {
  ...
  Eigen::Vector2d vel;
  Obstacle() {}
  Obstacle(double radius, double height, Eigen::Vector3d pos, Eigen::Vector2d vel)
      : radius(radius), height(height), pos(std::move(pos)), vel(std::move(vel)) {}
};
...
class SingleTrajRecorder {
 ...
 public:
  ...
  SingleTrajRecorder(ros::NodeHandle& nh) {
    nh_         = nh;
    agent_id_   = 0;
    topic_name_ = std::string("/SQ0s") + std::to_string(agent_id_) +
                  std::string("/goal");  // TODO: change to pose
  }
  ...
  void cmdCallback(const snapstack_msgs::GoalConstPtr& msg) {
    t_            = msg->header.stamp.toSec();
    position_     = Eigen::Vector3d(msg->p.x, msg->p.y, msg->p.z);
    velocity_     = Eigen::Vector3d(msg->v.x, msg->v.y, msg->v.z);
    acceleration_ = Eigen::Vector3d(msg->a.x, msg->a.y, msg->a.z);
    yaw_          = msg->psi;
    yaw_rate_     = msg->dpsi;
  }
